...
: QObject( parent )
{
    ...
    connect( d->http, SIGNAL( responseHeaderReceived(QHttpResponseHeader) ),
             SLOT( responseHeaderReceived(QHttpResponseHeader) ) );
}
...
#ifdef XMLRPC_DEBUG
    qDebug() << __FILE__ << __LINE__ << __FUNCTION__ << "xmlrpc client: set proxy" << host << port << userName << password;
#endif
...
int Client::request( QList<Variant> params, QString methodName )
{
    ...
    if (!d->cookie.isEmpty())
    {
        QListIterator<QString> iter(d->cookie);
        while(iter.hasNext())
            header.addValue("Cookie", iter.next());
    }
...
#ifdef XMLRPC_DEBUG
    qDebug()  << __FILE__ << __LINE__ << __FUNCTION__ << this << "xmlrpc request(" << id << "): " << methodName;
    qDebug()  << __FILE__ << __LINE__ << __FUNCTION__ << this << "Header:" << header.toString();
    qDebug()  << __FILE__ << __LINE__ << __FUNCTION__ << this << Variant(params).pprint();
#endif
...
#ifdef XMLRPC_DEBUG
    qDebug() << __FILE__ << __LINE__ << __FUNCTION__ << "request" <<  d->methodNames[id] <<  "finished, id=" << id << ", isError:" << error;
#endif
...
#ifdef XMLRPC_DEBUG
        qDebug()  << __FILE__ << __LINE__ << __FUNCTION__<< "xml-rpc server response:\n" << QString(buf);
#endif
...
#ifdef XMLRPC_DEBUG
                qDebug() << __FILE__ << __LINE__ << __FUNCTION__ << response.returnValue().pprint();
#endif
...
#ifdef XMLRPC_DEBUG
            qDebug() << __FILE__ << __LINE__ << __FUNCTION__ << "incorrect xmlrpc response:" << errorMessage;
            qDebug() << __FILE__ << __LINE__ << __FUNCTION__ << QString(buf);
#endif
...
void Client::responseHeaderReceived(QHttpResponseHeader header)
{
#ifdef XMLRPC_DEBUG
    qDebug() << __FILE__ << __LINE__ << __FUNCTION__ << "response header:\n" << header.toString();
#endif

    if (header.hasKey("Set-Cookie"))
        d->cookie.append(header.allValues("Set-Cookie"));

#ifdef XMLRPC_DEBUG
    qDebug() << __FILE__ << __LINE__ << __FUNCTION__ << "stored cookies:\n" << d->cookie;
#endif
}
