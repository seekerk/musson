 ...
 */
QString QDeclarativeFolderListModel::folder() const
{
    return d->folder;
}

void QDeclarativeFolderListModel::setFolder(const QString &folder)
{
    if (folder == d->folder)
        return;
    QModelIndex index = d->model.index(folder);
    if ((index.isValid() && d->model.isDir(index)) || folder.isEmpty()) {
...
QString QDeclarativeFolderListModel::parentFolder() const
{
    QString localFile = d->folder;
    if (!localFile.isEmpty()) {
        ...
        int pos = d->folder.lastIndexOf(QLatin1Char('/'));
        if (pos == -1)
            return QString();
        localFile = d->folder.left(pos);
    }
    return localFile;
}
...
void QDeclarativeFolderListModel::setSelected(QString file, bool isSelected)
{
    if (isSelected) {
        ...
        if (m_selectedFiles.contains(file))
            m_selectedFiles.removeAll(file);
    }
    emit selectedChanged();
}

void QDeclarativeFolderListModel::resetSelected()
{
    m_selectedFiles.clear();
    emit selectedChanged();
}

bool QDeclarativeFolderListModel::isSelected(QString file)
{
    return m_selectedFiles.contains(file);
}

QStringList QDeclarativeFolderListModel::selectedItems() const
{
